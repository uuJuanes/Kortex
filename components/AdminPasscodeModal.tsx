...
const AdminPasscodeModal: React.FC<AdminPasscodeModalProps> = ({ onClose, onVerify }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!success) {
      setError('Clave de acceso de administrador incorrecta.');
      setIsShaking(true);
      ...
      <div className={`bg-background-card rounded-xl shadow-lg p-6 w-full max-w-sm mx-4 border border-border-default transform transition-all duration-300 ${isShaking ? 'animate-shake' : ''}`}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-text-default flex items-center">
            <KeyIcon className="w-5 h-5 mr-3 text-text-muted" />
        ...
        <p className="text-sm text-text-muted mb-4">
          Se requiere autorización para cambiar a un perfil de administrador del sistema.
        </p>
